...
contract Airdrop is Ownable {
    ...
    /// @param v, r, s bytes representing a signature created by the
    /// `Airdrop.signer` address
    /// @param _to The address the claimed MACRO should be sent to
    function signatureClaim(address _to, uint256 _amount, uint8 v, bytes32 r, bytes32 s) external {
        require(!isECDSADisabled, "SIGS_DISABLED");
        require(alreadyClaimed[msg.sender] == false, "Airdrop: address already claimed");

        address signatory = ecrecover(toTypedDataHash(_to, _amount), v, r, s);
        require(signer == signatory, "Airdrop: not valid signer");
    ...
    function merkleClaim(bytes32[] calldata _proof, address _to, uint256 _amount) external {
        ...
        require(_merkleVerify(_proof, merkleRoot, leaf), "Airdrop: invalid Merkle proof");
        alreadyClaimed[msg.sender] = true;
    ...
    function _merkleVerify(bytes32[] memory proof, bytes32 root, bytes32 leaf) internal pure returns (bool) {
        bytes32 computedHash = leaf;

        for (uint256 i = 0; i < proof.length; i++) {
            bytes32 proofNode = proof[i];

            if (computedHash <= proofNode) {
                computedHash = keccak256(abi.encodePacked(computedHash, proofNode));
            } else {
                computedHash = keccak256(abi.encodePacked(proofNode, computedHash));
            }
        }
